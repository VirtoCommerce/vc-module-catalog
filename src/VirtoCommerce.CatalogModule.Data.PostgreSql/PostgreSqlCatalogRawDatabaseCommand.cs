...
namespace VirtoCommerce.CatalogModule.Data.PostgreSql
{
    public class PostgreSqlCatalogRawDatabaseCommand : ICatalogRawDatabaseCommand
    {
        ...
        public virtual async Task RemoveAllPropertyValuesAsync(CatalogDbContext dbContext, PropertyEntity catalogProperty, PropertyEntity categoryProperty, PropertyEntity itemProperty)
        {
            if (catalogProperty != null)
            {
                await dbContext.Database.ExecuteSqlInterpolatedAsync(
                    $"""
                     DELETE FROM "PropertyValue" PV
                     USING "Catalog" C WHERE C."Id" = PV."CatalogId" AND C."Id" = {catalogProperty.CatalogId} AND PV."Name" = {catalogProperty.Name};
                     """);
            }
            if (categoryProperty != null)
            {
                await dbContext.Database.ExecuteSqlInterpolatedAsync(
                    $"""
                     DELETE FROM "PropertyValue" PV
                     USING "Category" C WHERE C."Id" = PV."CategoryId" AND C."CatalogId" = {categoryProperty.CatalogId} AND PV."Name" = {categoryProperty.Name};
                     """);
            }
            if (itemProperty != null)
            {
                await dbContext.Database.ExecuteSqlInterpolatedAsync(
                    $"""
                     DELETE FROM "PropertyValue" PV
                     USING "Item" I WHERE I."Id" = PV."ItemId" AND I."CatalogId" = {itemProperty.CatalogId} AND PV."Name" = {itemProperty.Name};
                     """);
            }
        }
        ...
        public async Task RemoveItemsAsync(CatalogDbContext dbContext, IList<string> itemIds)
        {
            ...
            foreach (var itemIdsPage in itemIds.Paginate(_batchSize))
            {
        ...
        public virtual async Task<GenericSearchResult<AssociationEntity>> SearchAssociations(CatalogDbContext dbContext, ProductAssociationSearchCriteria criteria)
        {
            ...
            if (criteria.Take > 0)
            {
                ...
                querySqlCommand.Parameters.Add(new NpgsqlParameter("@skip", criteria.Skip));
                querySqlCommand.Parameters.Add(new NpgsqlParameter("@take", criteria.Take));
            }
