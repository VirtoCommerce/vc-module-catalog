...
namespace VirtoCommerce.CatalogModule.Data.BulkUpdate.Model.Actions.UpdateProperties
{
    public class UpdatePropertiesBulkUpdateAction : IBulkUpdateAction
    {
        ...
        {
            var dataQuery = _context.DataQuery as ProductBulkUpdateDataQuery
                ?? throw new InvalidCastException($"{nameof(Context)}.{nameof(UpdatePropertiesActionContext.DataQuery)} should be of \"{nameof(ProductBulkUpdateDataQuery)}\" type.");

            var properties = _bulkUpdatePropertyManager.GetProperties(dataQuery);
